...
public class ControllerClient extends Endpoint implements MessageHandler.Whole<String> {
	...
	private final ClientEndpointConfig cec = ClientEndpointConfig.Builder.
			create().encoders(Arrays.asList(CommandLineEncoder.class)).decoders(Arrays.asList(CommandLineDecoder.class)).build();
	...
	public void connect(URI uri) throws DeploymentException, IOException, URISyntaxException {
		try {
	...
	public void onMessage(String message) {
		if (initializing) {
			if (SessionManager.ACK.equals(message)) {
				...
				try {
					pshell.msgln("Server accept our control request.");
				} catch (IOException e) {
